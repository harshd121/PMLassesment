...
### Set the work environment and knitr options according to the given options 


```r
rm(list=ls(all=TRUE)) #start with empty workspace
startTime <- Sys.time() #for the startime again solutions given

library(knitr) #knit to r for pdf or md or rmd production
...
## Modeling
In random forests, there is no need for cross-validation or a separate test set to get an unbiased estimate of the test set error. It is estimated internally,Data modeling in software engineering is the process of creating a data model for an information system by applying certain formal techniques during the execution. So, we proceed with the training the model (Random Forest) with the training data set.
...
An examination, where the marks may or may not count, which serves chiefly as practice for future exams or so that the teachers are able to set a grade before the end of term.
